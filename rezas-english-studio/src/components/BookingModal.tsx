...
import { Calendar as CalendarIcon, Clock, CreditCard, User, Mail, Phone, MessageSquare, ArrowLeft } from 'lucide-react'
...
export function BookingModal({ isOpen, onClose, classType }: BookingModalProps) {
  ...
  const handleSubmit = () => {
    // Validate form
    if (!formData.name || !formData.email || (!selectedDate && classType !== 'offline')) {
  ...
  const handleOldSubmit = () => {
    ...
    toast.success('Booking confirmed! Payment processed successfully. You will receive a confirmation email shortly.')
  ...
  return (
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle className="text-2xl gradient-text">Book Your {currentClass.title}</DialogTitle>
          <DialogDescription>
        ...
        {currentStep === 'payment' ? (
          ...
          <div className="space-y-6">
            ...
            <Button onClick={handleSubmit} className="w-full glow-effect" size="lg">
